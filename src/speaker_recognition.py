...
def load_df(option):
    filelist = os.listdir('../Audios_dataset/' + option)
...
def extract_features(files, option=None, training=False):
    if training:
        file_name = os.path.join(os.path.abspath(
            '../Audios_dataset/') + '/' + option + '/' + str(files.file))
...
def prepare_data():
    ...
    train_features = train_df.apply(
        extract_features, option='train', training=True, axis=1)
    X_train, y_train = concatenate_features(train_df, train_features)
    val_features = validation_df.apply(
        extract_features, option='validation', training=True, axis=1)
...
def real_time(model):
    ...
    stream = p.open(format=pyaudio.paInt16, channels=2, rate=44100,
                    ...
                    frames_per_buffer=44100, input=True, output=False)
    ...
    while (True):
        if (c >= 7):
            ...
            print(np.where(confidence == True,
                  persons[predicciones], 'Unknown'))
            if confidence >= 0.95:
                change_led(True)
            audio_frames = []
            c = 0
